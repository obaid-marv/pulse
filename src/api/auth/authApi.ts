import { basicResponse, LoginResponse, MyDetailsResponse, SignupResponse } from "@/types/auth/interfaces";
import axiosInstance from "../axiosInstance";
import { baseUrl } from "../baseUrl";
import axios from "axios";
import Cookies from "js-cookie";

export const register = async (data: {email: string; password: string; name:string; username:string }): Promise<SignupResponse> => {
    const response = await axiosInstance.post(`${baseUrl}/api/v1/user`, data)
    return response.data
  };


export const verifyEmail = async (data: {email: string; code: string}): Promise<basicResponse> => {
